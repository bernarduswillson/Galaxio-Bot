    ...
    public void computeNextPlayerAction(PlayerAction playerAction) {
        if (this.gameState.world.getCurrentTick() != null && this.gameState.world.getCurrentTick() != 0 && this.gameState.world.getCurrentTick() > tickX) {
            ...
            isAction == 0
                ...
                playerAction.action = PlayerActions.FORWARD;
                System.out.println("FOOD WHILE BEING CHASED");
                runAwayF = 0;
                isAction = 1;
            }
        
            // 10. Finding best area to move to
            if (
            (scanFood != null && 
            ...
            currentTick % 20 == 0
                ...
                if (scanFood.size() > 0) {
                    ...
                    if (getDistanceBetween(closestScanFood, this.bot) < worldRadius * 0.25) {
            ...
            //11. Kamikaze when there is no more food
            if (
            closestFood == null && 
            isAction == 0
            ) {
            ...
            //12. Default food prevent bug under 5 ticks
            if (
            currentTick < 5 && 
            closestFood != null
            ) {
    ...
    private int scanObject() {
        ...
        for (int i = 0; i < 360/vision; i++) {
            ...
            for (int j = 0; j < gasCloudList.size(); j++) {
                scoreSum += -20 / getDistanceBetween(this.bot, gasCloudList.get(j));
            }
